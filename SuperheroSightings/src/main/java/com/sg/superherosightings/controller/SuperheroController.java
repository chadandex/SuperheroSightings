...
public class SuperheroController {
    ...
    public String editSighting(@Valid @ModelAttribute("sighting") Sighting sighting, BindingResult result, HttpServletRequest request) {
        ...
        // if statement to error check user
        // if user doesnt select any drop box to update
        if (locationIdParameter == null && heroIdParameter == null) {
            Location location = service.getLocationBySightingId(sighting.getSightingId());
            long locationId = location.getLocationId();
            Heroes hero = service.getHeroBySightingId(sighting.getSightingId());
            long heroId = hero.getHeroId();
            service.updateSighting(sighting, heroId, locationId);
        } else if (locationIdParameter == null) { // if nothing selected for location, update rest without new location
            int heroId = Integer.parseInt(heroIdParameter);
            Location location = service.getLocationBySightingId(sighting.getSightingId());
            long locationId = location.getLocationId();
            service.updateSighting(sighting, heroId, locationId);
        } else if (heroIdParameter == null ) { // if nothing selected for hero, update rest without new hero
            int locationId = Integer.parseInt(locationIdParameter);
            Heroes hero = service.getHeroBySightingId(sighting.getSightingId());
            long heroId = hero.getHeroId();
            service.updateSighting(sighting, heroId, locationId);
        } else { // all things checked to be updated. update as normal
            int heroId = Integer.parseInt(heroIdParameter);
            int locationId = Integer.parseInt(locationIdParameter);
            service.updateSighting(sighting, heroId, locationId);
        }
